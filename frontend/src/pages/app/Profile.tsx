...
export const Profile: React.FC = () => {
  ...
  const handleSendFeedback = async (e: React.FormEvent) => {
    ...
    if (!feedbackMessage.trim() || feedbackMessage.trim().length < 5) {
      setFeedbackError('Pesan masukan minimal 5 karakter.');
      return;
    }
    ...
    try {
      const res = await api.post<{ success: boolean }>('/user/feedback', {
      ...
      setFeedbackError(err.response?.data?.message || 'Gagal mengirim masukan. Silakan coba lagi.');
    } finally {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-5">
      ...
      {/* ── 3. Masukan Pengembang (Developer Feedback) ────────────────────── */}
      <div className="bg-cream/90 border border-rule rounded-xl p-6 space-y-4 shadow-sm text-left">
        <h3 className="font-display font-bold text-base text-ink">Kirim Masukan untuk Developer</h3>
        <p className="text-xs text-muted">
          Punya ide fitur baru, laporan bug, atau masukan untuk pengembangan proyek? Tuliskan di bawah ini agar pembuat dapat terus memperbarui sistem.
        </p>

        {feedbackSuccess && (
          <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-4 text-success text-xs font-semibold animate-pulse">
            Terima kasih! Masukan Anda telah sukses dikirim ke developer untuk pembaruan proyek.
          </div>
        )}
        {feedbackError && (
        ...
        <form onSubmit={handleSendFeedback} className="space-y-4">
          <div className="flex flex-col gap-1.5">
            <label htmlFor="feedback-category" className="text-xs font-bold text-slate-300 uppercase tracking-wide">
              Kategori Masukan
            </label>
            <select
              ...
              className={`w-full px-3 py-2 border-2 rounded-xl text-sm font-medium focus:outline-none transition-colors ${
                theme === 'light'
                  ? 'border-slate-350 focus:border-blue-500 text-slate-800 bg-white'
                  : 'border-white/10 focus:border-blue-500 text-white bg-[#0a0a0f]'
              }`}
            >
              <option value="general" className={theme === 'light' ? 'text-slate-800' : 'text-white bg-[#131318]'}>Umum / Saran</option>
              <option value="bug" className={theme === 'light' ? 'text-slate-800' : 'text-white bg-[#131318]'}>Laporan Bug / Masalah</option>
              <option value="feature" className={theme === 'light' ? 'text-slate-800' : 'text-white bg-[#131318]'}>Usulan Fitur Baru</option>
              <option value="subject" className={theme === 'light' ? 'text-slate-800' : 'text-white bg-[#131318]'}>Mata Pelajaran Belum Terdaftar</option>
            </select>
          </div>

          <div className="flex flex-col gap-1.5">
            <label htmlFor="feedback-message" className="text-xs font-bold text-slate-300 uppercase tracking-wide">
              Isi Masukan / Saran *
            </label>
            <textarea
              id="feedback-message"
              ...
              className={`w-full px-3 py-2 border-2 rounded-xl text-sm font-medium focus:outline-none transition-colors resize-none min-h-[100px] ${
                theme === 'light'
                  ? 'border-slate-350 focus:border-blue-500 text-slate-800 bg-white'
                  : 'border-white/10 focus:border-blue-500 text-white bg-[#0a0a0f]'
              }`}
              placeholder="Jelaskan secara detail ide, saran, atau keluhan mata pelajaran baru yang belum terdaftar di sini..."
            />
          </div>
          ...
          <button
            ...
            className="w-full py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-bold disabled:opacity-40 disabled:cursor-not-allowed transition-all"
          >
            {isSendingFeedback ? 'Mengirim Masukan...' : 'Kirim Masukan Sekarang'}
          </button>
        </form>
      </div>
