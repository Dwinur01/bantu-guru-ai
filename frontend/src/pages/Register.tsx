    ...
    <div className="min-h-screen relative overflow-hidden flex items-center justify-center p-4 animate-page bg-auth-gradient">
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="text-center mb-7 animate-fade-up">
          ...
          <h1 className="font-display font-black text-3xl text-white mb-2">Daftar Akun Guru</h1>
          <p className="text-slate-400 text-sm">Gratis selamanya untuk plan dasar 🎓</p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-7 shadow-2xl animate-fade-up-1">
          ...
          {successEmail ? (
            <div className="space-y-5 text-center animate-scale-in">
              ...
              <div>
                <h2 className="font-display font-black text-xl text-white mb-2">Registrasi Berhasil! 🎉</h2>
                <p className="text-sm text-slate-400 mb-4">Tautan verifikasi telah dikirim ke:</p>
                <div className="bg-blue-950/40 border border-blue-500/35 rounded-2xl p-3">
                  <span className="text-sm font-black text-brand-red select-all">{successEmail}</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              {/* Server Error */}
              {serverError && (
                <div className="flex items-start gap-3 p-3.5 bg-red-950/40 border border-red-500/30 rounded-2xl text-red-400 animate-scale-in">
              ...
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
                ...
                <div className="space-y-1.5">
                  <label htmlFor="reg-name" className="block text-sm font-bold text-slate-300">Nama Lengkap</label>
                  <div className="relative">
                    <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
                      <User className="w-4 h-4" />
                    </div>
                    <input id="reg-name" type="text" placeholder="John Doe" {...register('name')} disabled={isLoading}
                      className={`input-premium w-full pl-10 pr-4 py-3 text-sm text-white placeholder-slate-500 ${errors.name ? 'error' : ''}`} />
                  </div>
                  {errors.name && <p className="text-xs text-red-400 font-semibold flex items-center gap-1 pt-0.5"><AlertCircle className="w-3 h-3" />{errors.name.message}</p>}
                </div>

                {/* Email */}
                <div className="space-y-1.5">
                  <label htmlFor="reg-email" className="block text-sm font-bold text-slate-300">Email Guru</label>
                  <div className="relative">
                    <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none">
                  ...
                  {errors.email && <p className="text-xs text-red-400 font-semibold flex items-center gap-1 pt-0.5"><AlertCircle className="w-3 h-3" />{errors.email.message}</p>}
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {/* Password */}
                  <div className="space-y-1.5">
                    <label htmlFor="reg-password" className="block text-xs font-bold text-slate-300">Password</label>
                    <div className="relative">
                      ...
                      <input id="reg-password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" {...register('password')} disabled={isLoading}
                        className={`input-premium w-full pl-9 pr-9 py-2.5 text-sm text-white placeholder-slate-500 ${errors.password ? 'error' : ''}`} />
                    ...
                    {errors.password && <p className="text-[10px] text-red-400 font-semibold flex items-start gap-1 leading-tight"><AlertCircle className="w-3 h-3 flex-shrink-0 mt-0.5" />{errors.password.message}</p>}
                  </div>

                  {/* Confirm */}
                  <div className="space-y-1.5">
                    <label htmlFor="reg-confirm" className="block text-xs font-bold text-slate-300">Konfirmasi</label>
                    <div className="relative">
                      <div className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none"><Lock className="w-3.5 h-3.5" /></div>
                      <input id="reg-confirm" type={showConfirmPassword ? 'text' : 'password'} placeholder="••••••••" {...register('confirmPassword')} disabled={isLoading}
                        className={`input-premium w-full pl-9 pr-9 py-2.5 text-sm text-white placeholder-slate-500 ${errors.confirmPassword ? 'error' : ''}`} />
                    ...
                    {errors.confirmPassword && <p className="text-[10px] text-red-400 font-semibold flex items-start gap-1 leading-tight"><AlertCircle className="w-3 h-3 flex-shrink-0 mt-0.5" />{errors.confirmPassword.message}</p>}
                  </div>
                </div>

                {/* Terms checkbox */}
                <div className="flex items-start gap-2 pt-1 text-left">
                  <input id="reg-agree" type="checkbox" required className="w-4 h-4 text-brand-red border-white/10 bg-[#0a0a0f] rounded focus:ring-brand-red mt-0.5" />
                  <label htmlFor="reg-agree" className="text-[11px] text-slate-400 leading-normal">
                    Saya setuju dengan{' '}
